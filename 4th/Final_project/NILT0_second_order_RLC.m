...
NILT0_result(1) = - (1 ./ h) .*sum(real(G(poles./h).*residues.*poles));
X_prev = NILT0_result(1);
for i=1:length(tspan)
    if i>1
    X_prev =NILT0_result(i-1); %use the previous to approximate the next one
    end
    most_right_term = sum(X_prev);
    NILT0_result(i) =  - (1 ./ h) .*sum(real(G(poles./h).*residues.*poles - most_right_term));% -most_right_term));
end
